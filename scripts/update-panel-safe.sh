#!/usr/bin/env bash
set -euo pipefail

repo_root="$(git rev-parse --show-toplevel 2>/dev/null || true)"

if [ -z "$repo_root" ]; then
  echo "Error: Not inside a git repository."
  exit 1
fi

cd "$repo_root"

git config advice.addIgnoredFile false >/dev/null 2>&1 || true

install_dependencies() {
  echo "===> Installing dependencies..."
  if command -v pnpm >/dev/null 2>&1; then
    pnpm install
    return 0
  fi

  if grep -Rqs "\"link:\"\\|link:" "$repo_root/package.json"; then
    echo "Error: package.json contains link: dependencies. Install pnpm (recommended) and retry."
    echo "Tip: corepack enable && corepack prepare pnpm@latest --activate"
    exit 1
  fi

  npm install
}

run_migrations_only() {
  echo "===> Running DB upgrade..."
  npm run upgrade-db
  echo "===> Migration completed successfully!"
}

run_full_update() {
  echo "===> Using repo: $repo_root"
  local old_head
  local target_head
  old_head="$(git rev-parse --short HEAD 2>/dev/null || echo unknown)"

  echo "===> Fetching latest main..."
  git fetch origin main
  target_head="$(git rev-parse --short origin/main 2>/dev/null || echo unknown)"

  echo "===> Ignoring local tracked code changes and syncing directly to origin/main..."
  git merge --abort 2>/dev/null || true
  git rebase --abort 2>/dev/null || true
  git reset --hard origin/main

  install_dependencies
  echo "===> Panel updated successfully!"
  echo "===> HEAD before: ${old_head}"
  echo "===> HEAD now:    ${target_head}"
  echo "===> Run option 2 if you also need database migrations."
}

show_menu() {
  echo
  echo "CPanel Updater"
  echo "1) Update panel"
  echo "2) Run migration"
  echo "3) Exit"
  echo
  read -r -p "Select an option [1-3]: " choice

  case "${choice:-1}" in
    1)
      run_full_update
      ;;
    2)
      run_migrations_only
      ;;
    3)
      echo "===> Exit."
      exit 0
      ;;
    *)
      echo "Invalid option."
      exit 1
      ;;
  esac
}

if [ -t 0 ]; then
  show_menu
else
  run_full_update
fi
